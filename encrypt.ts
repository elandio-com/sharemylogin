...
export const MIN_PASSWORD_LENGTH = 16; // Strict policy: 16 chars required
export const MAX_DATA_SIZE = 15360; // 15KB - Strict limit for free tier
...
export async function encryptData(data: string, password: string): Promise<{
    ...
    if (data.length > MAX_DATA_SIZE) {
        throw new Error(`Data too large. Maximum size is ${Math.floor(MAX_DATA_SIZE / 1024)}KB`);
    }
